...
function show_lit(literal)
   io.write(string.format("%s(%s", literal.pred.id, show_term(literal[1])))
   for i=2,#literal do
      io.write(string.format(", %s", show_term(literal[i])))
   end
   io.write(")\n")
end
...
function fact(subgoal, literal)
   if not is_member(literal, subgoal.facts) then
      fact_depth = fact_depth + 1
      io.write(string.format("fact>   %d ", fact_depth))
      show_lit(literal)
...
function search(subgoal)
   ...
   if literal.pred.prim then
      ...
      io.write(string.format("search> %d ", search_depth))
      show_lit(literal)
...
-- Example of the addition primitive.
